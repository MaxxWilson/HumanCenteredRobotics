...
class ManipulatorInterface(Interface):
    ...
    def _compute_wbc_command(self):
        ...
        if(self._count < np.size(self.planned_traject["theta"]["time"])):
            ...
            M_x1 = pinv(j_x1 @ A_pinv @ j_x1_t)                             # [1x3]*[3x3]*[3x1] = scalar
            ...
            F_x1 = M_x1 * (x1_accel_des + kp1 * (x1_des - x1_i) + kd1 * (x1_vel_des - x1_vel_i) - self._robot.get_link_jacobian_dot_times_qdot("ee")[2])
            # F_x2 = M_x2_x1 @ (j_x2_x1 @ (x2_accel_des + kp2 * (x2_des - x2_i) + kd2 * (x2_vel_des - x2_vel_i)))

            [U, V, D] = np.linalg.svd(N_x1@A_pinv@N_x1@j_x2_x1_t)
            ...
            jtrq = j_x1_t @ F_x1 + (N_x1_t @ (x2_accel_des + kp2 * (x2_des - x2_i) + kd2 * (x2_vel_des - x2_vel_i))).reshape(3, 1) + (b+g).reshape(3, 1)
            # jtrq = j_x1_t @ F_x1 + (j_x2_x1_t @ F_x2 + b+g).reshape(3, 1)
            #jtrq = (j_x2_x1_t @ F_x2 + b+g).reshape(3, 1)
            ...
            plt.title("q1")
            plt.subplot(3, 1, 1)
            plt.plot(self.planned_traject["q1"]["time"], self.planned_traject["q1"]["pos"], linestyle='-')
            plt.plot(self.robot_traject["time"], self.robot_traject["q1"])
            plt.subplot(3, 1, 2)
            plt.plot(self.planned_traject["q1"]["time"], self.planned_traject["q1"]["vel"], linestyle='-')
            plt.plot(self.robot_traject["time"], self.robot_traject["q1_vel"])
            plt.subplot(3, 1, 3)
            plt.plot(self.planned_traject["q1"]["time"], self.planned_traject["q1"]["accel"], linestyle='-')

            plt.figure()
            plt.title("q2")
            plt.subplot(3, 1, 1)
            plt.plot(self.planned_traject["q2"]["time"], self.planned_traject["q2"]["pos"], linestyle='-')
            plt.plot(self.robot_traject["time"], self.robot_traject["q2"])
            plt.subplot(3, 1, 2)
            plt.plot(self.planned_traject["q2"]["time"], self.planned_traject["q2"]["vel"], linestyle='-')
            plt.plot(self.robot_traject["time"], self.robot_traject["q2_vel"])
            plt.subplot(3, 1, 3)
            plt.plot(self.planned_traject["q2"]["time"], self.planned_traject["q2"]["accel"], linestyle='-')
            
            plt.figure()
            plt.title("q3")
            plt.subplot(3, 1, 1)
            plt.plot(self.planned_traject["q3"]["time"], self.planned_traject["q3"]["pos"], linestyle='-')
            plt.plot(self.robot_traject["time"], self.robot_traject["q3"])
            plt.subplot(3, 1, 2)
            plt.plot(self.planned_traject["q3"]["time"], self.planned_traject["q3"]["vel"], linestyle='-')
            plt.plot(self.robot_traject["time"], self.robot_traject["q3_vel"])
            plt.subplot(3, 1, 3)
            plt.plot(self.planned_traject["q3"]["time"], self.planned_traject["q3"]["accel"], linestyle='-')
